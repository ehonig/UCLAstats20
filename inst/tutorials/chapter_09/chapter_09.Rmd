...
The reading ease score $\mathrm{RE}$ is a number between 0 and 100. Higher values of $\mathrm{RE}$ indicate text that is easier to read, and lower values indicate text that is more difficult to read.
...
```{r vowels, exercise = TRUE,exercise.setup = "prepare-vowels"}
...
```{r, echo = FALSE}
pre_process_text <- function(text) {
  text_sentences <- strsplit(text, split = "[.!?:;]")[[1]]
  text_sentences <- tolower(text_sentences)
  text_sentences <- gsub(pattern = "[[:punct:]]", replacement = "", text_sentences)
  words <- strsplit(text_sentences, split = " ")
  lapply(words, keep_words)
}

reading_ease <- function(text) {
  words <- pre_process_text(text)
  asl <- mean(vapply(words, length, numeric(1)))
  asw <- mean(vapply(unlist(words), count_syllables, numeric(1)))
